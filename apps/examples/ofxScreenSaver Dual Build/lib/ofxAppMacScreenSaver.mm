...
ofxAppMacScreenSaver::~ofxAppMacScreenSaver() {
    NSLog(@"ofxAppMacScreenSaver :: ~ofxAppMacScreenSaver: deallocating");
    if(ofAppPtr != NULL) {
        delete ofAppPtr;
        ofAppPtr = NULL;
    }
...
void ofxAppMacScreenSaver::setupOpenGL(int w, int h, int screenMode, string dataPath) {
    ...
    printf("ofxAppMacScreenSaver :: setupOpenGL : %d x %d \n", w, h);
...
//------------------------------------------------------------
void ofxAppMacScreenSaver::setup() {
    
    // we always want 60 fps //
    bFrameRateSet 			= true;
	float durationOfFrame 	= 1.0f / 60.f;
...
//------------------------------------------------------------
void ofxAppMacScreenSaver::exit_cb() {
    //ofNotifyExit();
    ofAppPtr->exit();
}

//------------------------------------------------------------
void ofxAppMacScreenSaver::keyPressed(int key ) {
    ofAppPtr->keyPressed(key);
}

//------------------------------------------------------------
void ofxAppMacScreenSaver::keyReleased(int key) {
